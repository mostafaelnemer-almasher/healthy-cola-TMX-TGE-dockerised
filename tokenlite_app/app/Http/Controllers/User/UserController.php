...
use App\Notifications\WalletChange;
...
class UserController extends Controller
{
    ...
    public function account_update(Request $request)
    {
        ...
        if ($type == 'personal_data') {
            ...
            if ($validator->fails()) {
                ...
                if ($validator->errors()->hasAny(['name', 'dateOfBirth'])) {
        ...
        if ($type == 'wallet_request') {
            ...
            if ($validator->fails()) {
                ...
                if ($is_valid) {
                    ...
                    $userId = auth()->id();
                    $meta_request = GlobalMeta::save_meta('user_wallet_address_change_request', json_encode($meta_data), $userId);
                    ...
                    if ($meta_request) {
                        try {
                            ...
                            if (!$chk_mail_stat) {
                                $user->notify(new WalletChange('pending', 'pending'));
                            }
                            ...
                            if ($to_all == 'all') {
                                Notification::send($admins, new WalletChange('pending-admin', 'pending', $user));
                            } elseif ($admin) {
                                $admin->notify(new WalletChange('pending-admin', 'pending', $user));
                            }
     ...
     * @return void
     */
    public function change_email_modal(Request $request)
    {
        $user = User::find(auth()->user()->id);
        $gm = GlobalMeta::where(['name' => 'new_change_email', 'pid' => $user->id])->first();        
        $new_email = isset($gm->value) ? $gm->value : null ;

        return view('modals.change-email', compact('user', 'new_email'))->render();
    }

    public function change_email_update(Request $request) {
        $validator = Validator::make($request->all(), [
            'user_new_change_email' => "required|email|regex:/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,9}$/ix|max:190|not_in:" . auth()->user()->email . "|unique:users,email," . auth()->user()->id,
        ], [
            'user_new_change_email.not_in' => __("The new email address cannot be the same as your current email address."),
            'user_new_change_email.unique' => __("The chosen email is already registered with us. Please use a different email address."),
            'user_new_change_email.regex' => __("Please enter a valid email address."),
        ]);

        if ($validator->fails()) {
            $msg = __('messages.form.wrong');
            if ($validator->errors()->hasAny(['user_new_change_email'])) {
                $msg = $validator->errors()->first();
            }
        ...
        $user = User::find(auth()->user()->id);
        GlobalMeta::save_meta('new_change_email', $request->user_new_change_email, $user->id);
        ...
        try {
            $sendTo = $request->input('user_new_change_email');
            
            Mail::to($sendTo)->send(new ChangeEmailConfirm($user));
            $ret['msg'] = 'success';
            $ret['message'] = __('messages.password.changed');
        } catch (\Exception $e) {
            $ret['msg'] = 'warning';
