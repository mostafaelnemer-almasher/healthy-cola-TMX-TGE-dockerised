...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($type == 'site_info') {
            ...
            Setting::updateValue('email_verification_disable', ($request->email_verification_disable == '1') ? 1 : 0);
            Setting::updateValue('mail_to_unverified_user', (isset($request->mail_to_unverified_user) ? 1: 0));
            ...
            $validator = Validator::make($request->all(), [
                ...
                'joined_signup_bonus' => 'integer|gte:0',
                'referred_signup_bonus' => 'integer|gte:0'
            ]);

            if ($validator->fails()) {
                $msg = '';
                if ($validator->errors()->hasAny(['referral_bonus', 'referral_bonus_join', 'joined_signup_bonus', 'referred_signup_bonus' ])) {
                    $msg = $validator->errors()->first();
                } else {
                ...
                Setting::updateValue('referral_required', (isset($request->referral_required) ? 1 : 0));
                Setting::updateValue('joined_signup_bonus', $request->input('joined_signup_bonus'));
                Setting::updateValue('referred_signup_bonus', $request->input('referred_signup_bonus'));
